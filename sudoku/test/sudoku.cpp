...
TEST(adrianbier,
     initialize) { // neues sudoku korrekt mit dem symbol für 0 gefüllt ist.
  Sudoku<9> sudoku;
  EXPECT_EQ(sudoku.get(0, 0), '_');
  EXPECT_EQ(sudoku.get(8, 8), '_');
}

TEST(adrianbier, set_and_get_value) { // gültige werte setzen und abfragen,
                                      // set()=true wenn richtig
  Sudoku<9> sudoku;
...
TEST(adrianbier,
     reject_invalid) { // set() sollte das erkennen und 'false' zurückgeben.
  Sudoku<9> sudoku;
...
// Test 1: Prüfen, ob ein neues Sudoku-Feld erstellt und mit 0 ("_") befüllt
// wird
...
// Test 2: Setzen und Lesen eines gültigen Wertes   ----> muss noch in der
// sudoku.hpp implementiert werden
...
TEST(BastHalb, Valid_input) {
  ...
  for (size_t i = 0; i < 9; ++i) {
    EXPECT_TRUE(
        sudoku.set(0, i, '1' + i)); // Set valid symbols in the first row
  }
}
...
// checking if sudoku has correctly initialized. All field should be equal to
// "_"
...
// checking if sudoku allows to enter symols which are meant for bigger fields
...
// setting a cell to a valid character should return true and getting it should
...
// setting multiple cells with valid characters and getting it should return the
// last character set
...
// setting a cell to a valid character at the edge of the Sudoku should work
// correctly
...
// setting a cell to a valid character and then overwriting it with another
// valid character should update the value correctly
...
// setting a cell to an invalid character should return false and not change the
// value of the cell
...
// setting a cell to a character that is valid for a smaller Sudoku but not for
// the current size should return false and not change the value of the cell
...
template <typename T> class dumberger_diff_size : public ::testing::Test {};

using Sizes = ::testing::Types<
    std::integral_constant<int, 1>, std::integral_constant<int, 4>,
    std::integral_constant<int, 9>, std::integral_constant<int, 16>,
